<@dwz.layout_content>
	...
	<@dwz.fieldset title="自定义授权">
		...
		<@dwz.fieldset title="${p_index + 1}.${p.name }">
			<@dwz.label_input content="名称：">
				<input type="text" name="name" class="required" size="32" maxlength="32" alt="请输入模块名称" value="${p.name }" readOnly=readOnly/>
			</@dwz.label_input>	
			<@dwz.label_input content="短名：">
				<input type="text" name="priority" class="required digits" size="32" alt="请输入短名" value="${p.shortName }" readOnly=readOnly/>
			</@dwz.label_input>						
			<@dwz.label_input content="描述：">
